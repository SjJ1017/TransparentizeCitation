...
import string
import time
def load_json(path, return_list = False):
    ...
    if isinstance(path, list):
        assert return_list, "path is a list, but return_list is False"
        return [load_json(p, return_list = True) for p in path]
    if not return_list:
        return loaded
    else:
        return loaded
...
def remove_citations(text):
    # 匹配 \cite[m]{n} 或者 \cite{n} 的模式
    pattern_cite = r'\\cite\[\d+\]{\d+}|\\cite{\d+}'
    # 匹配 [n] 的模式
    pattern_square_brackets = r'\[\d+\]'
    
    # 先去除 \cite[m]{n} 和 \cite{n}
    text = re.sub(pattern_cite, '', text)
    # 再去除 [n]
    text = re.sub(pattern_square_brackets, '', text)
